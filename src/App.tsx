...
export default function App() {
  ...
  const onFiles = (files: File[]) => run(async () => {
    ...
    for (const file of files) {
      ...
      const result = await request<{ path: string; link: string }>('attachment_save', { notePath: doc.path, name: file.name, base64: btoa(binary), image });
      let link = result.link;
      if (image && latest.current.settings.uploadEnabled) {
        try {
          link = (await request<{ url: string }>('attachment_upload', { path: result.path, endpoint: latest.current.settings.uploadEndpoint })).url;
        } catch (error) {
          notify(`本地附件已保存，图床上传失败：${error instanceof Error ? error.message : String(error)}`);
        }
      }
      editor.current?.insert(`${image ? '!' : ''}[${file.name.replace(/[\[\]]/g, '')}](${link})\n`);
    }
  });
